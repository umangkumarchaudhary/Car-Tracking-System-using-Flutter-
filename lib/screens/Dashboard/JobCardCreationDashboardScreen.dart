...
class JobCardCreationDashboardScreen extends StatefulWidget {
  final String authToken;
  final String dashboardTitle;

  const JobCardCreationDashboardScreen({Key? key, required this.authToken, required this.dashboardTitle})
      : super(key: key);

  @override
  State<JobCardCreationDashboardScreen> createState() => _JobCardCreationDashboardScreenState();
}

class _JobCardCreationDashboardScreenState extends State<JobCardCreationDashboardScreen> {
  String filter = "today";
  ...
  Future<void> fetchDashboardData() async {
    ...
    try {
      final url = Uri.parse("http://192.168.58.49:5000/api/dashboard/job-card-creation?filter=$filter");
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                ...
                Text("Creation Start Time: ${formatTime(vehicle['creationStartTime'])}"),
                Text("First Received Time: ${formatTime(vehicle['firstReceivedTime'])}"),
                Text("Creation Performed By: ${vehicle['creationPerformedBy'] ?? 'Unknown'}"),
                Text("Received Performed By: ${vehicle['receivedPerformedBy'] ?? 'Unknown'}"),
                Text("Duration: ${vehicle['durationMinutes'] ?? '0'} minutes"),
  ...
  Widget _buildVehicleTile(Map<String, dynamic> vehicle, bool isActive) {
    return Card(
      ...
      margin: const EdgeInsets.symmetric(vertical: 6, horizontal: 8), // Added horizontal margin
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(15), // More rounded corners
        side: BorderSide(color: Colors.blueGrey.shade100, width: 0.7),
      ),
      child: InkWell(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      "Created by: ${vehicle['creationPerformedBy'] ?? '-'} | Received by: ${vehicle['receivedPerformedBy'] ?? '-'}",
                      style: TextStyle(
                        fontSize: 14,
                    ...
                    Row(
                      ...
                      children: [
                        Text(
                          "Duration: ${vehicle['durationMinutes'] ?? '0'} mins",
                          style: const TextStyle(
                            fontSize: 14,
          ...
          children: [
            ...
            GridView.count(
              ...
              children: [
                ...
                _buildStatItem(Icons.timer, "Avg. Duration", "${summary['averageDurationMinutes'] ?? '0'} mins"),
                  ...
                  children: [
                    ...
                    if (showCompleted) ...[
                      ...
                      if (completedVehicles.isEmpty)
                        const Text("No completed job cards.")
                      else
